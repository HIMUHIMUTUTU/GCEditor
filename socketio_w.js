...
io.sockets.on('connection', function(socket) {
  ...
  /** Register garbages **/
  socket.on('garbageRegister', function(data) {
	  console.log("receive garbage data:");
	  console.dir(data.value);
	  
	  //convert JSON object to String 
	  var garbage = JSON.stringify(data.value.garbage);
	  
	  referlog.insertgarbages(garbage, function(err, data){
		  if(err){
			  console.log(err);
		  }else{
  ...
  /** Send garbage data to client **/
  socket.on('garbageFetch', function(data) {
	  var id = data;
	  referlog.selectgarbages(id, function(garbage_data){
			console.log("SELECT COMPLETE");
			garbage_data = JSON.parse(garbage_data[0].contents);	  
			 
			  //send it to agent
			  socket.emit('getgarbage', { value: garbage_data });
