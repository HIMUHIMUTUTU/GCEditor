...
var frame = {
	draw: {x: 0, y: 50, w: 640, h: 750},
	config: {x: 0, y: 0, w: 1280, h: 10},
	gomi: {x: 640, y: 50, w: 640, h: 750}
};
...
socket.on('getgarbage', function(msg) {
...
//for mobile same as PC
canvaswrap.ontouchend = function(event) { self.mouseup(event); };
canvaswrap.ontouchstart = function(event) { self.mousedown(event); };
canvaswrap.ontouchmove = function(event) { self.mousemove(event); };
...
MAIN.prototype.draw = function(event){
  ...
  if(inFrame(pos.x, pos.y, frame.draw)){
	  ...
	  this.line[this.line_num].display(this.cc);
  }
};
...
/* garbage register loop */
MAIN.prototype.garbageRegister = function(){
			var stroke = new Array();
			var gr = 0;
			for(var li = this.recoggarbage; li < this.line.length; li++){
				stroke[gr] = this.line[li];
				gr++;
			};
			if(gr != 0){
				this.recoggarbage = this.line.length; //register recorded line
				socket.emit('garbageRegister', {value: {garbage: {stroke: stroke}}});
				console.log({value: {garbage: {stroke: stroke}}});
			}
};

/* garbage fetch loop */
MAIN.prototype.garbageFetch = function(){
	socket.emit('garbageFetch', {value: 1});
};
