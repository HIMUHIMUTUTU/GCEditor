...
function MAIN(){
	...
	//function for close bottun click
	closeButton.onclick  = function(event) {self.close()};
	...
	if (navigator.onLine == true) {
		...
		common.socket.on('connect', function(msg) {
			self.network= "online";
			self.networkStatus.innerHTML= "接続中"; 
			self.initiateEditor();
			//authentication
		...
		common.socket.on('disconnect', function(data) {
			...
			self.networkStatus.innerHTML= "オフライン (データは一時的にPCに保存されますが、オンラインでの利用をおすすめします)"; 
		});
		//if offline
		}else{
			this.initiateEditor();
